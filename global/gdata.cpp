...
void GData::removeFileFromList(SoundFile *s)
{
  ...
  for(std::vector<SoundFile*>::iterator it2=soundFiles.begin(); it2 != soundFiles.end(); ) {
    if((*it2) == s) {
      it2 = soundFiles.erase(it2);
    }
    else {
        it2++;
    }
...
int GData::saveFile(SoundFile *s, QString newFilename)
{
  ...
  int ret = (moveFile(oldFilename, newFilename)) ? 0 : -1;
  if(ret == 0) {
	s->stream->open_read(newFilename);
...
int GData::closeFile(SoundFile *s, int theSavingMode/*, bool ask*/)
{
  ...
  oldFilename = QDir::toNativeSeparators(oldFilename);
  QByteArray oldFilenameUtf8 = oldFilename.toUtf8();
  const char* oldFilenameStr = oldFilenameUtf8.constData();
  ...
  if(theSavingMode == ALWAYS_ASK) {
    QString filename = QFileInfo(oldFilenameStr).completeBaseName();
    ...
    case 0: //Yes
      ...
      if(newFilename.isNull()) return 1;
      {
          removeFileFromList(s);
          delete s;
          ret = (::moveFile(oldFilename, newFilename)) ? 0 : -1;
      }
      break;
    case 1: //No
      removeFileFromList(s);
      delete s;
      //printf("remove file %s\n", oldFilenameStr); fflush(stdout);
      ret = ::remove(oldFilenameStr);
      break;
    default: //Cancelled
      return 1;
    }
