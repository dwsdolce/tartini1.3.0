  ...
  : QWidget(parent)
{
    ...
    if (mode == 0) {
      ...
      compass->setMode(QwtCompass::RotateNeedle);
      compass->setScaleMaxMajor(11);
      compass->setScaleMaxMinor(2);
      compass->setScaleStepSize(30);
      for (int i = 0; i < 12; i++) {
        notes[i * 30] = noteName(i);
      }
      compass->setScaleDraw(new QwtCompassScaleDraw(notes));
    }
    QwtAbstractScaleDraw *sd = compass->scaleDraw();
    if (sd) {
        sd->setTickLength(QwtScaleDiv::MinorTick, 1);
        sd->setTickLength(QwtScaleDiv::MediumTick, 1);
