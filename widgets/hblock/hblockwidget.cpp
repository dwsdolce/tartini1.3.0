...
void HBlockWidget::paintEvent( QPaintEvent * )
{
  ...
  if(active) {
    ...
    if(theData) {
      ...
      QColor fillColor = colorBetween(palette().color(QPalette::WindowText), active->color, data.volumeValue());
      QColor outlineColor = colorBetween(palette().color(QPalette::WindowText), Qt::black, data.volumeValue());
