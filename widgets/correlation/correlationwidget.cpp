...
void CorrelationWidget::paintEvent( QPaintEvent * )
{
  ...
  p.setPen(QPen(colorBetween(palette().color(QPalette::Window), Qt::black, 0.3), 0));
  ...
  if(active) { 
    ...
    if(data && (aggregateMode == 0)) {
      ...
      if(data->highestCorrelationIndex >= 0 && !data->periodEstimatesAmp.empty()) {
      ...
      if(data->chosenCorrelationIndex >= 0 && !data->periodEstimatesAmp.empty()) {
        p.setPen(Qt::red);
        p.setBrush(Qt::red);
