...
fast_smooth::fast_smooth(size_t size)
{
    _size = size;
    _size_left = _size / 2;
...
void fast_smooth::fast_smoothA(float *source, float *dest, size_t length, size_t step)
{
	...
	dest[j*step] = float((total_sum - cos_sum) / _sum);
...
void fast_smooth::fast_smoothA(float *source, float *dest, size_t length)
{
    //blur stays centered if odd
    double cos_sum = 0;
    double sin_sum = 0;
    ...
    for(j=0; j<_size_left; j++) {
	dest[j] = float((total_sum - cos_sum) / _sum);
...
void fast_smooth::fast_smoothB(float *source, float *dest, size_t length, size_t step)
{
...
//treats all values off either end the same as the value at end
void fast_smooth::fast_smoothB(float *source, float *dest, size_t length)
{
  //blur should be odd
  double cos_sum = 0;
  double sin_sum = 0;
  ...
  size_t j;
  if(length < _size) {
    float avg = float(std::accumulate(source, source+length, 0.0) / float(length));
